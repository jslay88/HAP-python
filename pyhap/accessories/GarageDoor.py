...
class DoorSwitch(object):
    ...
    def setup_gpio(self):
        ...
        GPIO.setup(self.pin, GPIO.IN, pull_up_down=self._pud)
        GPIO.add_event_detect(self.pin, self._edge)
...
class GarageDoor(Accessory):
    ...
    def __init__(self, relay_pin, switch_pin, *args,
        ...
        self.relay.setup_gpio()
        self.switch.setup_gpio()
        super(GarageDoor, self).__init__(*args, **kwargs)

    def __getstate__(self):
        state = super().__getstate__()
        state.update(relay=None, switch=None)
        return state
    ...
    def door_closing(self):
        self.current_state_characteristic.set_value(3)
        self.target_state_characteristic.value = 1
        self.target_state_characteristic.notify()

    def door_closed(self):
        self.current_state_characteristic.set_value(1)
        self.target_state_characteristic.value = 1
        self.target_state_characteristic.notify()

    def door_open(self):
        self.current_state_characteristic.set_value(0)
        self.target_state_characteristic.value = 0
        self.target_state_characteristic.notify()

    def door_opening(self):
        self.current_state_characteristic.set_value(2)
        self.target_state_characteristic.value = 0
        self.target_state_characteristic.notify()
